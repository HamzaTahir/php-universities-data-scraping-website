...
$html = file_get_html('https://bahria.edu.pk/academics/under-graduate-programs/bs-computer-science/');
...
// 'province_name', 'city_name', 'uni_name', 'field'
	...
	$Main_Query="SELECT * FROM uni_details_table
				 WHERE province_name='Punjab' AND city_name='Lahore' AND field='BS(CS)'AND uni_name='Bahria University' ";
	...
	if(mysqli_num_rows($Result)!=0){
		$update_query="UPDATE uni_details_table SET uni_logo='".$image->src."',
			   ...
			   Semester_7='".$roadmap[6]."',Semester_8='".$roadmap[7]."'
			    WHERE province_name='Punjab' AND city_name='Lahore' AND field='BS(CS)'AND uni_name='Bahria University' ";
		if (mysqli_query($conn,$update_query)) {
			echo "Updated Bahria University BS(CS) Field Data Scraped From It's Website And Stored In Database";
		}
		else{
		...
		$insert_query="INSERT INTO uni_details_table(province_name,city_name,uni_logo,uni_name,field,
					   ...
					   VALUES('Punjab','Lahore','".$image->src."','Bahria University','BS(CS)',
					   '".$title->plaintext."','".$description->plaintext."','".$roadmap[0]."'
					   ,'".$roadmap[1]."','".$roadmap[2]."','".$roadmap[3]."','".$roadmap[4]."'
					   ,'".$roadmap[5]."','".$roadmap[6]."','".$roadmap[7]."')";
		if (mysqli_query($conn,$insert_query)) {
			echo "Bahria University Data BS(CS) Field Scraped From It's Website And Stored In Database";
		}
		else{
