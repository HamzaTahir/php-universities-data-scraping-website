...
$html = file_get_html('http://lhr.nu.edu.pk/cs/programDetails/BS%20(Computer%20Science)');
...
if (!$conn) {
	...
	$Main_Query="SELECT * FROM uni_details_table
				 WHERE province_name='Punjab' AND city_name='Lahore' AND field='BS(CS)'
				 AND uni_name='FAST' ";
	...
	if(mysqli_num_rows($Result)!=0){
		$update_query="UPDATE uni_details_table SET uni_logo='".$image."',
			   ...
			   WHERE province_name='Punjab' AND city_name='Lahore' AND field='BS(CS)'AND uni_name='FAST' ";
		if (mysqli_query($conn,$update_query)) {
			echo "Updated FAST University BS(CS) Field Data Scraped From It's Website And Stored In Database";
		}
		else{
		...
		$insert_query="INSERT INTO uni_details_table(	
					   ...
					   VALUES('Punjab','Lahore','".$image."','FAST','BS(CS)',
					   '".$title."','".$description."','".$roadmap."')";
		
		if (mysqli_query($conn,$insert_query)) {
			echo "FAST University Data BS(CS) Field Scraped From It's Website And Stored In Database";
		}
		else{
