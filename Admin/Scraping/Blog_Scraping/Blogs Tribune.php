...
$html = file_get_html('https://blogs.tribune.com.pk/tag/pakistani-culture/');
...
if (!$conn) {
   ...
   for ($i=0; $i < 10 ; $i++) { 
	...
	for ($i=0; $i < 10 ; $i++) { 
		...
		// echo "Hello Kesy ho Yr :: ".$story_image;
	...
	$Main_Query="SELECT story_link FROM post_table 
				 WHERE story_link='".$story_link."' ";

	$Result=mysqli_query($conn,$Main_Query);

	if(mysqli_num_rows($Result)==0){
		$insert_query="INSERT INTO post_table(	
